...
const services = [
  ...
  {
    icon: (
      <svg viewBox="0 0 48 48" width="26" height="26"><rect width="48" height="48" rx="8" fill="var(--pr-blue)"/><path d="M14 14h8c3.3 0 6 2.7 6 6s-2.7 6-6 6h-5v8h-3V14zm3 3v6h5c1.7 0 3-1.3 3-3s-1.3-3-3-3h-5z" fill="white"/><path d="M30 14h7c2.2 0 4 1.8 4 4 0 1.5-.8 2.8-2 3.5L42 30h-3.5l-2.7-7.5H33V30h-3V14zm3 3v4h4c.8 0 1.5-.7 1.5-1.5S37.8 17 37 17h-4z" fill="white" opacity="0.7"/></svg>
    ),
    ...
    >
      <button
        ...
        className={`w-full text-left rounded-2xl border transition-all duration-300 ${
          service.featured
            ? "border-[var(--pr-green)]/40 bg-[var(--pr-green)]/5 hover:bg-[var(--pr-green)]/8"
            : "border-white/[0.06] bg-[var(--pr-navy)] hover:border-white/[0.12]"
          ...
          >
            <div
              className={`px-5 pb-5 pt-3 rounded-b-2xl border border-t-0 ${
                service.featured ? "border-[var(--pr-green)]/40 bg-[var(--pr-green)]/5" : "border-white/[0.06] bg-[var(--pr-navy)]"
              }`}
              style={{ boxShadow: `0 8px 30px -8px ${service.borderGlow}` }}
            >
              <p className="text-[var(--text-muted)] text-sm leading-relaxed pl-14">
                {service.desc}
              </p>
              {service.featured && (
                <div className="pl-14 mt-3">
                  <a
                    href="#contact"
                    className="inline-flex items-center gap-1.5 text-sm font-medium text-[var(--pr-green)] hover:underline"
                  >
                    Get started →
                  </a>
                </div>
    ...
    >
      ...
      <div className="px-7 py-5 border-b border-white/[0.06] flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[var(--text-muted)] text-sm mt-0.5">We&apos;ll get back to you within 1 business day.</p>
        </div>
        <a href="tel:0490881483" className="hidden sm:flex items-center gap-2 text-[var(--pr-blue)] text-sm font-medium hover:underline flex-shrink-0">
          <Phone size={14} />
          0490 881 483
        </a>
      </div>
      ...
      {sent ? (
        <div className="px-7 py-10 text-center">
          <div className="w-12 h-12 rounded-full bg-[var(--pr-green)]/15 flex items-center justify-center mx-auto mb-3">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M5 13l4 4L19 7" stroke="var(--pr-green)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
          </div>
          <p className="text-white font-semibold text-lg">Message sent!</p>
          <p className="text-[var(--text-muted)] text-sm mt-1">We&apos;ll be in touch shortly.</p>
        </div>
      ) : (
        <form onSubmit={onSubmit} className="px-7 py-6 grid sm:grid-cols-2 gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-[var(--text-muted)] text-xs font-medium uppercase tracking-wider">Your Name *</label>
            <input name="name" required value={form.name} onChange={onChange} placeholder="John Smith"
              className="px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white text-sm placeholder-[var(--text-faint)] focus:outline-none focus:border-[var(--pr-blue)]/50 transition-colors" />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-[var(--text-muted)] text-xs font-medium uppercase tracking-wider">Phone Number *</label>
            <input name="phone" type="tel" required value={form.phone} onChange={onChange} placeholder="04XX XXX XXX"
              className="px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white text-sm placeholder-[var(--text-faint)] focus:outline-none focus:border-[var(--pr-blue)]/50 transition-colors" />
          </div>
          <div className="flex flex-col gap-1.5 sm:col-span-2">
            <label className="text-[var(--text-muted)] text-xs font-medium uppercase tracking-wider">Service Interested In</label>
            <select name="service" value={form.service} onChange={onChange}
              className="px-4 py-3 rounded-xl bg-[var(--pr-navy)] border border-white/[0.08] text-white text-sm focus:outline-none focus:border-[var(--pr-blue)]/50 transition-colors appearance-none">
          ...
          <div className="flex flex-col gap-1.5 sm:col-span-2">
            <label className="text-[var(--text-muted)] text-xs font-medium uppercase tracking-wider">Message (optional)</label>
            <textarea name="message" value={form.message} onChange={onChange} rows={3} placeholder="Tell us a bit about your business..."
              className="px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white text-sm placeholder-[var(--text-faint)] focus:outline-none focus:border-[var(--pr-blue)]/50 transition-colors resize-none" />
          </div>
          <div className="sm:col-span-2">
            <button type="submit"
              ...
              style={{ background: "linear-gradient(135deg, var(--pr-blue) 0%, #0044CC 100%)" }}
            >
